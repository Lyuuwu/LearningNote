...
- S: the set of internal states of M
- I: the set of input
- O: the set of output
...
**Transient state**  
A transient state $s$ indicate that there is no $x$ for which $v(s, x)=s$. Consequently, after transition, state s is not reachable from any other state.  

**Sink state**  
A sink state $s$ indicates that the state is trapped. Consequently, for all inputs, there is no transition occurrence to sink state $s$.  
